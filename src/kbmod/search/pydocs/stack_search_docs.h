...
static const auto DOC_StackSearch_search = R"doc(
  ...
  GPU searches support up to ``kbmod.search.MAX_NUM_IMAGES`` images (inclusive) with
  sigma-G filtering enabled, and up to ``kbmod.search.MAX_NUM_IMAGE_TIMES`` images (inclusive)
  without sigma-G filtering. Larger stacks raise a ``RuntimeError``. CPU searches do not have
  a fixed image-count limit.
