...
namespace search {
  ...
  LayeredImage::LayeredImage(const RawImage& sci, const RawImage& var,
                             const RawImage& msk, const PSF& psf)
    : psf(psf) {
  ...
  LayeredImage::LayeredImage(std::string name, unsigned h, unsigned w,
                             float noise_stdev, float pixel_variance,
                             double time, const PSF& psf)
    : LayeredImage(name, h, w, noise_stdev, pixel_variance, time, psf, -1) {}
  ...
  LayeredImage::LayeredImage(std::string name, unsigned h, unsigned w,
    ...
    search::Image tmp = search::Image::NullaryExpr(h, w, gaussian);
    science = RawImage(tmp, time);
    mask = RawImage(h, w, 0.0);
    variance = RawImage(h, w, pixel_variance);
  }
