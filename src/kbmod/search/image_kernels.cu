...
__global__ void device_basic_shift_and_stack(int num_images, int width, int height,
                                             float *image_vect, float* times,
                                             float x_v, float y_v, bool use_mean,
                                             float *results) {
    // Get the pixel coordinates to use.
    const int x = blockIdx.x * blockDim.x + threadIdx.x;
    const int y = blockIdx.y * blockDim.y + threadIdx.y;
    if (x >= width || y >= height) return;

    // Loop over each image and compute the co-add.
    float sum = 0.0;
    float count = 0.0;
    for (int t = 0; t < num_images; ++t) {
        // Predict the trajectory's position.
        float cTime = times[t];
        int img_x = int(x + x_v * cTime);
        int img_y = int(y + y_v * cTime);

        // Get the pixel value.
        if ((img_x >= 0) && (img_x < width) && (img_y >= 0) && (img_y < height)) {
            int pixel_index = width * height * t + img_y * width + img_x;
            if (image_vect[pixel_index] != NO_DATA) {
                sum += image_vect[pixel_index];
                count += 1.0;
            }
        }
    }
    ...
    // Save the result
    if (use_mean) {
        results[y * width + x] = (count > 0.0) ? sum / count : 0.0;
    } else {
        results[y * width + x] = sum;
    }
}
...
void deviceBasicShiftAndStack(ImageStack *stack, float x_v, float y_v, bool use_mean, float *results) {
    ...
    const unsigned int width = stack->getWidth();
    const unsigned int height = stack->getHeight();
    const unsigned int num_image_pixels = width * height;
    const unsigned int total_pixels = num_images * num_image_pixels;

    // Allocate and copy the times.
    checkCudaErrors(cudaMalloc((void **)&device_times, sizeof(float) * num_images));
    checkCudaErrors(cudaMemcpy(device_times, stack->getTimesDataRef(), sizeof(float) * num_images,
                               cudaMemcpyHostToDevice));

    // Allocate and copy the images into a single vector.
    checkCudaErrors(cudaMalloc((void **)&device_img, sizeof(float) * total_pixels));
    float *next_ptr = device_img;
    for (unsigned t = 0; t < num_images; ++t) {
        const std::vector<float> &data_ref = stack->getSingleImage(t).getScience().getPixels();
        checkCudaErrors(cudaMemcpy(next_ptr, data_ref.data(), sizeof(float) * num_image_pixels,
                                   cudaMemcpyHostToDevice));
        next_ptr += num_image_pixels;
    }
    ...
    // Read back results
    checkCudaErrors(cudaMemcpy(results, device_res, sizeof(float) * num_image_pixels,
