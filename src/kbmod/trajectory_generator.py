...
from kbmod.filters.clustering_grid import apply_trajectory_grid_filter
...
def generate_all_trajectories(
    ...
    *,
    ...
    """Generate all trajectories from a TrajectoryGenerator and apply the grid filter to remove
    duplicates if needed.

    Parameters
    ----------
    trj_generator : `TrajectoryGenerator`
        The TrajectoryGenerator to generate trajectories from.
    candidate_dup_px : `int`, optional
        The number of pixels to use for duplicate filtering. Default is 0 (no filtering).
    max_dt : `float`, optional
        The maximum time difference to consider for duplicate filtering. Default is 1.0.

    Returns
    -------
    candidates : `list`
        A list of all trajectories generated by the TrajectoryGenerator.
    """
    logger.debug(f"Trajectory Generator: {trj_generator}")
    candidates = [trj for trj in trj_generator]
    if candidate_dup_px > 0:
        logger.debug(
            f"Generated {len(candidates)} candidates per pixel and filtering with "
            f"candidate_dup_px={candidate_dup_px}."
        )
        candidates = apply_trajectory_grid_filter(
            candidates,
            bin_width=candidate_dup_px,
            max_dt=max_dt,
        )[0]
    logger.debug(f"Generated {len(candidates)} candidates per pixel.")
