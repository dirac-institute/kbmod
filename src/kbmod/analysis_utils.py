...
class PostProcess(SharedTools):
    ...
    def apply_clustering(self, keep, cluster_params):
        ...
        Arguments:
            keep (dict): Contains the values of which results were kept from the
                search algorithm
            cluster_params (dict): Contains values concerning the image and
                search initial settings including: x_size, y_size, vel_lims,
                ang_lims, and mjd.

        Returns:
            dict: A results dictionary as generated by SharedTools.gen_results_dict.
        """
        rs = ResultList(cluster_params["mjd"])
        rs.append_result_dict(keep)
