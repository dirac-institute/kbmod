...
class JointFit:
    def __init__(
        self, stamps, variances, dates, stamp_center_radec, stamp_center_pixel,
        psfs, wcs_list):
        ...
        for i in range(self.j):
            with solar_system_ephemeris.set('de432s'): #https://docs.astropy.org/en/stable/coordinates/solarsystem.html
                ...
                obs_pos = SkyCoord(earth_pos.x+obs_pos.x, earth_pos.y+obs_pos.y, earth_pos.z+obs_pos.z, representation_type='cartesian')
                self.obs_pos_list.append(obs_pos)

    def geo_to_bary_fast(self, c, i):
        c.representation_type = 'cartesian'
        earth_pos = self.earth_pos_list[i]
        c = SkyCoord(c.x + earth_pos.x, c.y + earth_pos.y, c.z + earth_pos.z,
    ...
    def obs_to_bary_fast(self, c, i):
        ...
        c = SkyCoord(c.x + obs_pos.x, c.y + obs_pos.y, c.z + obs_pos.z,
                     representation_type='cartesian')
        c.representation_type = 'spherical'
        return c
    ...
    def geo_to_bary_wdist(self, c, i, dist):
        ...
        bary_c = SkyCoord(earth_pos.x + r * c.x, earth_pos.y + r * c.y, earth_pos.z + r * c.z, representation_type='cartesian')
    ...
    def obs_to_bary_wdist(self, c, i, dist):
        ...
        bary_c = SkyCoord(obs_pos.x + r * c.x, obs_pos.y + r * c.y, obs_pos.z + r * c.z, representation_type='cartesian')
        bary_c.representation_type = 'spherical'
        return bary_c
    ...
    def model_traj_from_geo(self, x1, y1, x2, y2, bary_dist):
        ...
        for i in range(self.j):
            c = SkyCoord(c1b.ra + m_ra*(self.dates[i]-self.dates[0]), c1b.dec + m_dec*(self.dates[i]-self.dates[0]), distance=bary_dist*u.au)
    ...
    def model_images(self, traj):
        ...
        for i in range(self.j):
            mdl[i] = shift(self.psfs[i], [traj[i,1]-self.stamp_pos[i,1], traj[i,0]-self.stamp_pos[i,0]])
        return mdl
...
#        return mdl
    ...
    def model_images_streaked(self,traj):                                       
        ...
        for i in range(num_img):
            ...
            for ii in range(num_img):
                streaked_psf += scipy.ndimage.shift(self.psfs[ii], (traj[ii,1] - traj[i,1], traj[ii,0] - traj[i,0]))
            streaked_psf/= num_img                                                   
                                                                                
            mdl[i] = scipy.ndimage.shift(self.psfs[i] - streaked_psf, [traj[i,1]-self.stamp_pos[i,1], traj[i,0]-self.stamp_pos[i,0]])
        return mdl 
    ...
    def model_images_streaked_2(self,traj):                                     
        ...
        for i in range(nimg):                                                   
            ...
            streakedPSF[rely:rely+stampsize, relx:relx+stampsize] += shift(self.psfs[i], (y[i] % 1, x[i] % 1)) / nimg
    ...
    def model_traj_topo_pv(self, ra_m, dec_m, v_ra, v_dec):
        ...
        pix = np.zeros((self.j,2))
        for i in range(self.j):
            c = SkyCoord(ra_m + v_ra*(self.dates[i]-mean_date), dec_m + v_dec*(self.dates[i]-mean_date), unit='deg')
            w = self.wcs_list[i]
    ...
    def compare_traj(self, traj):
        ...
        for i in range(self.j):
            ...
            plt.scatter([self.window+traj[i,0]-self.stamp_pos[i,0]], [self.window+traj[i,1]-self.stamp_pos[i,1]], marker='x', c='r', s=200)
        plt.show()
...
def shift_images(traj, stamps, stamp_pos):
    ...
    for i in range(len(stamps)):
        shifted_stamps.append(shift(stamps[i], [-traj[i,1]+stamp_pos[i,1], -traj[i,0]+stamp_pos[i,0]]))
