...
class CreateStamps(object):
    ...
    def plot_all_stamps(
        self, results, lc, lc_index, coadd_stamp, stamps, stamp_index=-1,
        sample=False, compare_SNR=False, show_fig=True):
        ...
        res_line = results
        title_string = 'Pixel (x,y) = ({:4.0f}, {:4.0f}), Vel. (x,y) = ({:8.2f},{:8.2f}), Lh = {:6.2f}, Index = {:2.0f}'
        ax[0,1].set_title(title_string.format(res_line['x'], res_line['y'],
            res_line['vx'], res_line['vy'], res_line['lh'], stamp_index))
...
class VisualizeResults:
    ...
    def compare_filter_effectiveness(self):
        ...
        for ccd in ccd_num:
            try:
                stamps = self.show_pg_results(190,ccd,cutoff=0.75, suffix='XSEDE', plot_stamps='none', lh_lim=15)
                num_results['cnn_lh15'].append(len(stamps))
                stamps = self.show_pg_results(190,ccd,cutoff=0.75, suffix='XSEDE', plot_stamps='none', lh_lim=10)
                num_results['cnn_lh10'].append(len(stamps))
                stamps = self.show_pg_results(190,ccd,cutoff=0.0, suffix='XSEDE', plot_stamps='none', lh_lim=15)
                num_results['lh15'].append(len(stamps))
                stamps = self.show_pg_results(190,ccd,cutoff=0.0, suffix='XSEDE', plot_stamps='none', lh_lim=10)
    ...
    def recover_known_objects(self,
        results_dir_format, im_dir_format, known_object_data_path, cutoff=0.75, suffix='XSEDE'):    
        ...
        for pgccd in known_data:
            ...
            try:
                ...
                keep_idx,results,stamper,stamps,all_stamps,lc_list,psi_list,phi_list,lc_index = load_stamps(results_dir,im_dir,suffix)
            ...
            if np.count_nonzero(results) != 0:
                ...
                stamps_fig, object_found, found_idx = stamper.target_results(
                    ...
                    vel_tol=5, atol=5, title_info='pg_num={:03d}, ccd={:02d}'.format(*pgccd.astype(int)))
                if object_found:
                    num_found_objects+=1
    ...
    def plot_init(self):
        with self.out:
            ...
            self.stamps_fig = self.stamper.plot_all_stamps(
                self.results[i], np.array(self.lc_list)[i], np.array(self.lc_index)[i],
                np.copy(self.stamps[i]), np.copy(self.all_stamps[i]),
                stamp_index=i, compare_SNR=False, show_fig=False)
            ...
            if len(self.good_idx) > 1:
                ...
                self.stamps_fig_next = self.stamper.plot_all_stamps(
                    ...
                    np.copy(self.stamps[i]), np.copy(self.all_stamps[i]),
                    stamp_index=i, compare_SNR=False, show_fig=False)
        self.counter.value='{:3d}/{:3d} | {:2d}'.format(self.current_index+1, len(self.good_idx), self.current_ccd)
    ...
    def save_current_fig(self,b):
        self.stamps_fig.savefig('./saved_trajectory_images/{:03d}_{:02d}_{}_{:07d}'.format(
    ...
    def show_pg_results(self,
        pg_num, ccd_num, suffix='current', plot_stamps='coadd', cutoff=0.0,
        lh_lim=10.0, index=None, use_widget=False):
        ...
        if np.count_nonzero(self.results) != 0:
            if plot_stamps=='coadd':
                ...
                if index is None:
                    if use_widget is False:
                        for i in self.good_idx:
                            stamps_fig = self.stamper.plot_all_stamps(
                                self.results[i], np.array(self.lc_list)[i], np.array(self.lc_index)[i],
                                np.copy(self.stamps[i]), np.copy(self.all_stamps[i]),
                                stamp_index=i, compare_SNR=False)
                    ...
                    stamps_fig = self.stamper.plot_all_stamps(
                        self.results[index], np.array(self.lc_list)[index], np.array(self.lc_index)[index],
                        np.copy(self.stamps[index]), np.copy(self.all_stamps[index]), stamp_index=index)
