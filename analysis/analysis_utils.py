...
class PostProcess(SharedTools):
    ...
    def apply_clipped_average(
        ...
        print('Clipped average filtering keeps %i results' %(len(keep['results'])))
        print('{:.2f}s elapsed'.format(time_elapsed))
        return(keep)

    def _clipped_average(
        self, psi_curve, phi_curve, index, num_clipped=4, n_sigma=4,
        lower_lh_limit=-100):
        """
        ...
        outlier_index = np.where(max_lh > median+n_sigma*sigma)
        if len(outlier_index[0])>0:
            outliers = np.min(max_lh[outlier_index])
