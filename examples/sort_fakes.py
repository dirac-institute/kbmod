...
def match_fakes(fake_df, res_df):
    fake_coords = np.array(
        [fake_df["x"].values, fake_df["y"].values, fake_df["x_final"].values, fake_df["y_final"].values],
        dtype=np.int,
    ).T
    res_coords = np.array(
        [res_df["x"].values, res_df["y"].values, res_df["x_final"].values, res_df["y_final"].values],
        dtype=np.int,
    ).T
...
def compare_fakes(image_dir, results_dir, results_suffix_list, time_file):
    ...
    for results_suffix in results_suffix_list:
        ...
        fake_df = pd.read_csv(
            os.path.join(results_dir, "results_fakes_%s.txt" % results_suffix),
            delimiter=" ",
            names=["x", "y", "xv", "yv", "flux", "mag"],
            skiprows=1,
        )
        ...
        for f_c, f_v in zip(fake_coords, fake_vel):
            fake_traj_coords.append(
                np.array([f_c[0] + f_v[0] * image_times, f_c[1] + f_v[1] * image_times]).T
            )
        ...
        for r_c, r_v in zip(res_coords, res_vel):
            res_final_coords.append(
                np.array([r_c[0] + r_v[0] * image_times[-1], r_c[1] + r_v[1] * image_times[-1]])
            )
        ...
        match_idx = np.where(dist <= 10.0)[0]
