...
class test_result_data_row(unittest.TestCase):
    def setUp(self):
        ...
        self.rdr = ResultRow(self.trj, self.times)
...
class test_result_list(unittest.TestCase):
    ...
    def test_append_single(self):
        ...
        for i in range(5):
            t = trajectory()
            t.lh = float(i)
            rs.append_result(ResultRow(t, self.times))
    ...
    def test_extend(self):
        ...
        for i in range(5):
            t = trajectory()
            t.lh = float(i)
            rs1.append_result(ResultRow(t, self.times))

        # Fill a second Result set with 5 different rows.
        for i in range(5):
            t = trajectory()
            t.lh = float(i) + 5.0
            rs2.append_result(ResultRow(t, self.times))
