...
def region_search(path, tfile):
    ...
    results = search.region_search(
        params["x_vel"], params["y_vel"], params["radius"], params["min_lh"], int(params["img_count"] / 2)
    )
...
def run(path, tfile, search_type):
    ...
    matched, unmatched = kb.match_trajectories(results, t_list, params["vel_error"], params["pixel_error"])
    ...
    print("Score: " + str(kb.score_results(results, t_list, params["vel_error"], params["pixel_error"])))
    # print timings
