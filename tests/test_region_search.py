...
class TestRegionSearch(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.default_datasetTypes = [
            dt.name for dt in self.butler.registry.queryDatasetTypes()
        ][:2]
    ...
    def test_init(self):
        ...
        rs = region_search.RegionSearch(
            MOCK_REPO_PATH, [], [], butler=self.butler, fetch_data_on_start=False
        )
    ...
    def test_get_dataset_type_freq(self):
        ...
        """
        freq = self.rs.get_dataset_type_freq(
            butler=self.butler, collections=self.default_collections
        )
