...
for i in range(16, len(orbids)):
    ...
    if check_results_no_filter > 0:
        ...
        wu.config.set("result_filename", f"./both/{oid}_pencil.ecsv")

        r = SearchRunner().run_search_from_work_unit(wu)
        # with torch.no_grad():
        #   torch.cuda.empty_cache()
        num_results_both = len(r)
        r_check = r[
            (r["x"] >= xs[0] - 1) & (r["x"] <= xs[0] + 1) & (r["y"] >= ys[0] - 1) & (r["y"] <= ys[0] + 1)
        ]
        check_results_both = len(r_check)

    else:
        num_results_both, check_results_both = None, None
