...
for i in range(16, len(orbids)):
    ...
    if check_results_no_filter > 0:
        wu.config.set("peak_offset_max", 1)
        wu.config.set("result_filename", f"./peak1/{oid}_pencil.ecsv")
        ...
        num_results_peak, check_results_peak = None, None
    ...
    df = pd.DataFrame(l).T
    df.to_csv("./summary4.csv", index=False, header=False, mode="a")
