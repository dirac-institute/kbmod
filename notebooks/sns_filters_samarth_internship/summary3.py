...
for i in range(20, len(orbids)):
    ...
    cols = [
        "oid",
        "total_num_results",
        "num_results_no_filter",
        "check_results_no_filter",
        "num_results_siG_clust",
        "check_results_sigG_clust",
        "mean_dx",
        "max_dx",
        "min_dx",
        "max_dx_diff",
    ...
    gen = {
        ...
        "vx_steps": 25,
        "min_vx": mean_vx - 0.125,
        "max_vx": mean_vx + 0.125,
        "vy_steps": 25,
        "min_vy": mean_vy - 0.125,
        "max_vy": mean_vy + 0.125,
    }
    ...
    wu.config.set("save_config", False)
    ...
    if check_results_no_filter > 0:
        wu.config.set("do_clustering", True)
        wu.config.set("sigmaG_filter", True)
        wu.config.set("result_filename", f"./neither/{oid}_pencil.ecsv")

        r = SearchRunner().run_search_from_work_unit(wu)
        # with torch.no_grad():
        #   torch.cuda.empty_cache()
        num_results_siG_clust = len(r)
        r_check = r[
            (r["x"] >= xs[0] - 1) & (r["x"] <= xs[0] + 1) & (r["y"] >= ys[0] - 1) & (r["y"] <= ys[0] + 1)
        ]
        check_results_siG_clust = len(r_check)

    else:
        num_results_siG_clust, check_results_siG_clust = None, None
    ...
    df = pd.DataFrame(l).T
    df.to_csv("./summary3.csv", index=False, header=False, mode="a")
